...
bool S3fsCurl::UploadMultipartPostCallback(S3fsCurl* ossfscurl)
{
  ...
  // check etag(md5);
  // change to upper case (Bryan Zhu)
  if(NULL == strstr(ossfscurl->headdata->str(), upper(ossfscurl->partdata.etag.c_str()).c_str())){
    return false;
  }
  ossfscurl->partdata.etaglist->at(ossfscurl->partdata.etagpos).assign(ossfscurl->partdata.etag);
  ossfscurl->partdata.uploaded = true;
...
S3fsCurl* S3fsCurl::UploadMultipartPostRetryCallback(S3fsCurl* ossfscurl)
{
  ...
  if(0 != newcurl->UploadMultipartPostSetup(ossfscurl->path.c_str(), part_num, upload_id)){
    DPRN("Could not duplicate curl object(%s:%d).", ossfscurl->path.c_str(), part_num);
    delete newcurl;
    return NULL;
  }
  return newcurl;
...
int S3fsCurl::ParallelMultipartUploadRequest(const char* tpath, headers_t& meta, int fd, bool ow_sse_flg)
{
  ...
  if(0 != (result = ossfscurl.PreMultipartPostRequest(tpath, meta, upload_id, ow_sse_flg))){
    close(fd2);
    return result;
  }
...
int S3fsCurl::PutHeadRequest(const char* tpath, headers_t& meta, bool ow_sse_flg)
{
  ...
  for(headers_t::iterator iter = meta.begin(); iter != meta.end(); ++iter){
    ...
    if(key == "Content-Type"){
      ...
      requestHeaders = curl_slist_sort_insert(requestHeaders, string(key + ":" + value).c_str());
    }else if(!ow_sse_flg && key == "x-oss-server-side-encryption"){
...
int S3fsCurl::UploadMultipartPostRequest(const char* tpath, int part_num, string& upload_id)
{
  ...
  if(0 == (result = RequestPerform())){
    // check etag
    // change to upper case (Bryan Zhu)
    if(NULL != strstr(headdata->str(), upper(partdata.etag.c_str()).c_str())){
...
bool S3fsMultiCurl::SetS3fsCurlObject(S3fsCurl* ossfscurl)
{
  ...
  if(cMap_all.end() != cMap_all.find(ossfscurl->hCurl)){
    return false;
  }
  cMap_all[ossfscurl->hCurl] = ossfscurl;
  return true;
}
  ...
  while(NULL != (msg = curl_multi_info_read(hMulti, &remaining_messages))){
    ...
    if(ossfscurl){
      ...
      if(CURLE_OK == msg->data.result){
        long responseCode = -1;
        if(ossfscurl->GetResponseCode(responseCode)){
          if(400 > responseCode){
            // add into stat cache
            if(SuccessCallback && !SuccessCallback(ossfscurl)){
              DPRN("error from callback function(%s).", ossfscurl->url.c_str());
            }
          ...
          DPRN("failed a request(Unknown respons code: %s)", ossfscurl->url.c_str());
        }
      }else{
