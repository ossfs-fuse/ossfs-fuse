...
bool PageList::Serialize(CacheFileStat& file, bool is_output)
{
  ...
  if(is_output){
    ...
    off_t total = ossfs_strtoofft(oneline.c_str());
    ...
    while(getline(ssall, oneline, '\n')){
      ...
      off_t offset = ossfs_strtoofft(part.c_str());
      // size
      if(!getline(ssparts, part, ':')){
        is_err = true;
        break;
      }
      off_t size = ossfs_strtoofft(part.c_str());
      // init
      if(!getline(ssparts, part, ':')){
        is_err = true;
        break;
      }
      bool is_init = (1 == ossfs_strtoofft(part.c_str()) ? true : false);
      // add new area
